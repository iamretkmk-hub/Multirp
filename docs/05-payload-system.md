...
| `trackers` 🆕 | The character's own trackers, the story-wide ones, and anything of theirs that has crossed a public threshold. Split back out of `scene_now` (it had been folded in since v26). **(!)** The owner tag (`Nerve (Emre)`) is load-bearing on the dashboard and in the director's list, where several people's trackers share a column — and noise in the reader's own group, so `render(owner,{own:true})` drops it there. The `YOUR PERSONAL TRACKERS` sub-heading is likewise printed only when a story-wide or public group is present to separate it from; alone, it merely restates the block heading. |
...
| `quests` | Active quests this character is named in. **(!)** A quest is written *about* the world, so it names the reader and speaks of them in the third person — "Emre knows more about it than he has said" lands in Emre's own payload. The prose is **not** rewritten (deciding which "he" is the reader guesses wrong eventually, and silently, and would wreck every other person named); instead the intro states plainly that the name in there is them. Progress stamps read `Day N` to match the rest of the payload, and the description is trimmed with `briefDesc` rather than cut mid-word. |
...
## v113.1 — the warning that reported the data instead of the template
...
could actually have typed in Settings. This was never tuner-specific: any engine whose injected data
contains `{{…}}` was producing the same noise.
...
`heatNarrBlock()` / `heatNarrShort()` feed `{{narr}}` in `heat_format` and `{{narr_short}}` in
`rail_heat_narr`, so the two can never disagree about which mode the beat is in. `heat_guidance`
