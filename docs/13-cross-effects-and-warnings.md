...
| Setting a background engine to `rp:true` | Sampling penalties derail its JSON. Engines run provider **sampling** defaults by design; never reach for `rp:true` to get an engine thinking — that is what its own card is for (v66.1). |
| Adding a `chatCompletion` call whose `fn` bucket is not the card supplying its model | The user adjusts one card and a different agent changes. Invisible while buckets held only a temperature; v66.1 gave them a thinking switch and it surfaced as "I turned thinking on for Memory and my Auto-RP narrator started thinking". `tests/agent-reasoning.browser.js` now fails on any crossed call site. |
| Gating an engine's explicit answer behind a turn/step floor | A floor belongs on the PROMPT (don't end this early) not on the engine (ignore what you were told). `runSceneWriter` read `resolved && ev.turn>=ev.minTurns`, so a writer resolving a granted ask on turn 2 of 3 had its answer discarded, the event stayed open, and turn 3 resolved the same tension a second time with a near-duplicate narration. |
| An exact-text guard used as a duplicate guard | Two accounts of one event are near-duplicates, never identical. The arc tracker can close the same arc more than once in a scene; `commitMemoryArc`'s exact-match check let every one of them through. Compare by word overlap within the same day and period (`memNearDuplicate`). |
| Deriving a time of day from the WORD a line uses | "akşamüstü, saat beş gibi" was filed by matching "akşam" while the stated hour was never read, so two consumers of one sentence could land in different slots. One table (`periodForHour` / `statedHour`); the clock beats the word wherever both appear. |
| Persisting a model's free-text place name | One agreed meeting was filed at three different places because each consumer resolved "salon" for itself (living room / hall / venue). Match against the world's registered places (`_matchKnownPlace`, which never creates one) and store only a match; an unmatched name is a flag, not a location. |
| Storing text written AT a character as a memory OF them | The memory bank is first person; a reckoning is written in the second ("You told yourself…"). Stored verbatim it put two grammatical persons in one list and handed every later prompt a line addressing its reader. `toFirstPerson()` on the way in. |
| Detecting a foreign word by "ASCII-only token" in a Turkish story | Most of Turkish is ASCII — bir, ve, gibi, kadar — so that test flags the whole sentence and every beat retries. Use a curated set of English words of four letters or more that have no Turkish homograph (`foreignWordHits`), lowercase only so proper nouns are exempt, and retry at most once. |
| Measuring a call's duration from the log timestamp | `dbg()` stamps `t` before it walks the payload to estimate tokens, and a caller may await something between logging and dispatching — both get charged to the request. Stamp `sentAt` immediately before the fetch (`dbgDispatched`) and measure from that, per attempt. |
| Deciding who owns a record from **presence now** rather than from the span | `commitMemoryArc` filtered the memory cast by `presentIds(chat)` at commit time, so a character who had just left got no memory of the scene they had just lived — and leaving is the beat that closes an arc, so the two always raced and the departure always won. Read participation off the span (`m.present`), never off the live roster. |
| Adding a pass to `onPeriodChanged` and assuming End Day has its own | It bails on a day roll. `reconcilePeriodFor` relied on that promise and End Day never kept it, so the final stretch of every day went unconsolidated forever. If a pass must run per period, run it from **both** hooks and make it idempotent. |
| Passing a day stamp to a background pass without the period | `endDayBackground` runs after `gameDay` and `period` have already advanced, so any `chatPeriod(chat)` fallback inside it returns the NEW day's period. Capture both before the roll and pass both. |
| Referencing a helper behind `typeof fn==="function"` | The guard turns a typo into a permanent silent default. `relOf` was called three times in the psyche engine and defined **nowhere**, so for its whole life the drives writer got no relationship readings, no settled view, and a signature that never moved when the feelings did — while `feelingsBlock`, one call site away, read the same pair correctly through `relObj`. If a helper is meant to exist, call it unguarded and let it throw. |
| Reusing `directorContext(chat,"gm")` for something that is not the gamemaster | That mode is built to AUTHOR EVENTS. The drives writer asked for it and got the world's directing notes, every offstage character's wants and pressure points, the venue list, four other people's private trackers and the who-is-who matrix — under a heading promising only where the character is and who can see, and contradicting its own prompt's rules to write about one person, decide nothing and invent nothing. Use `"psyche"` (scene + earshot) or add a mode. |
| Matching a place cue against the SUB-AREA name only | The activity belongs to the venue; the area is furniture. `_outfitAreaActivity` read `sub.name`, so "Site Fitness Centre / Free Weights Zone" named no sport, and a venue with no sub-areas resolved to "Entrance" — swim/sport/sleep could not fire anywhere in the world. Fixed v67.1: area first, venue when the area says nothing. |
| Turning **Thinking: On** for an engine bucket with a tight `fnTok` cap | The cap covers reasoning *and* answer. `chatCompletion` raises it to ≥2000 when thinking is on, but a parser that expects a 60-token JSON line now waits on a model that thinks first — slower, pricier, and likelier to wrap the JSON in prose. Turn it on where the *judgment* is the product (gamemaster, daily engines, authoring), not where the output is one line of JSON (routers, trackers, presence). |
...
| Naming a person in a payload via `ownerLabel()` | It answers the literal `"(unknown)"` for a deleted or unresolvable id — correct as a Settings badge, poison in a prompt (`• [(unknown) — his oldest friend]` hands the model a nameless person to invent). It is also truthy, so `if(!who)` guards sail past it. Payload-facing code uses **`castLabel()`**, which returns `""` so the caller drops the entry; `ownerLabel` stays for the UI. |
| Detecting a "stale format" by the SHAPE of a reply | A narration-block-then-dialogue shape is not an artifact, it is an ordinary house format that FORMAT RULES can be edited to *require*. `_recentVoiceMarkup` used to match it and shipped the voice-reset block every turn, telling the model the formatting in its own history was stale. Detect a format artifact only by something that cannot occur naturally — bracketed delivery tags inside quotes. |
| Putting a POSITION rule in `rail_form` | It is the last line of the payload and outweighs everything; a position rule there ("do not open with scene-setting") contradicts any format edited to require an opening narration. `rail_form` governs volume; FORMAT RULES governs shape and position. |
